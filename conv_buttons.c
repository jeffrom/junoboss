...
int
convCC_SX_btn(Byte parambyte, Byte valuebyte, MIDIPacket *pktToSend)
{
    int cnvStatus = 0;
    int sxparamtosend = 0;
...
int
convSX_CC_btn(Byte parambyte, Byte valuebyte, MIDIPacket *pktToSend)
{
    int retval = 0;
...
Byte
convGetCCValueByte(struct btn_conversion_buffer *pbuf, Byte sxValByte)
{
    int retval = 0;
    if (pbuf->cc_value_range_end == 0 && pbuf->cc_value_range_start == 0) {
        /* this is a nongroup, non special button, this can only turn
	   these buttons on for now. */
        retval = 127;
    }
...
/* if this gets called, we have already checked that the button isn't
   on already.  these two funx must return 1 if they change something,
...
/* this should only ever have to turn off a single bit, never a range
   of index numbers (group) also it shouldn't ever have to check for any
   other buttons' states (so far :D) */
int
conv_BtnOff(struct btn_conversion_buffer *pb, Byte *vbyte)
{
    int retval = 1;
...
display_btns()
{
    for (unsigned int i = 0; i < number_of_button_params; i++) {
        printf("[%d]%s CC- %d SX- %d G- %d IG- %d OS- %d IS- %d IE- %d CCS- %d CCE- %d ",
	       ...
	       btn_conv_buf[i].cc_param_number,
	       btn_conv_buf[i].sx_param_number,
	       btn_conv_buf[i].group,
	       btn_conv_buf[i].ingroup,
	       btn_conv_buf[i].onstate,
	       btn_conv_buf[i].index_start,
	       btn_conv_buf[i].index_end,
	       btn_conv_buf[i].cc_value_range_start,
	       btn_conv_buf[i].cc_value_range_end);
...
int
calc_cc_end(int gamt, int gmemb)
{
