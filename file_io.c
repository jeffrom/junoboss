...
int
GetDefaultMIDIInterfaces(MIDIUniqueID *HostID, MIDIUniqueID *SynthID)
{
...
int
SetDefaultMIDIInterfaces(MIDIUniqueID HostID, MIDIUniqueID SynthID)
{
...
int
convGetDefaultSetting(const char *section, const char *setting, void **paddy)
{
    static int d_retInt;
    int retType = CONV_NEXTPROP_ERR;
    while (!feof(conversionfile)) { /* find section name then setting name */
...
/* figure out how many parameters in a section */
int
convGetNumberOfProperties(const char *section)
{
    int i = 0;
    /* char line[1024]; */
...
int
convGetNextProperty(const char *section, void **paddy)
/* this could stand to have some error checking subroutines */
{
 ...
 checktoken:
	...
	if (token != NULL)
		...
		/* this is a string.  remove quotes load the string,
		   return.  this should be done in a
		   subroutine. GetNextProperty should be a void function
		   pointer that either runs a string or int capturing
		   subroutine that returns a pointer to the int/char
		   array */
		...
		*paddy = (Byte *)&retInt;
		...
		if (!strncmp(token, section, strlen(section))) {
		    /* if this is the section we're supposed
		       to be in skip a line and goto token
		       parsing dont have to skip bc it's at
...
char*
colon_blow(char *token, const char delims[MAX_DELIMS], size_t size)
{
